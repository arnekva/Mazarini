...
const mazariniCCG_series: ILootSeries = {
    ...
    common: mazariniCCG.filter((c) => c.rarity === ItemRarity.Common).map((c) => c.id),
    rare: mazariniCCG.filter((c) => c.rarity === ItemRarity.Rare).map((c) => c.id),
    epic: mazariniCCG.filter((c) => c.rarity === ItemRarity.Epic).map((c) => c.id),
    legendary: mazariniCCG.filter((c) => c.rarity === ItemRarity.Legendary).map((c) => c.id),
    hasColor: false,
