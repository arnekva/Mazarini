...
import { mastermindButtons, mastermindContainer } from '../../templates/containerTemplates'
...
export class Mastermind extends AbstractCommands {
    ...
    private async setupMastermind(interaction: ChatInputCommandInteraction<CacheType>) {
        ...
        const msg = await this.messageHelper.replyToInteraction(interaction, '', undefined, [container.container])
        const buttons = mastermindButtons()
        this.messageHelper.sendMessage(interaction.channelId, { components: [buttons.container] }, { isComponentOnly: true })
        game.message = msg
    ...
    private updateGameMessage(interaction: ButtonInteraction<CacheType>) {
        ...
        // const currentRowIndex = game.container.getComponentIndex(`guess${(game.previousGuesses?.length ?? 0) + 1}`)
    ...
    private async updateUserStats(game: MMGame) {
        ...
        user.dailyGameStats = { ...user.dailyGameStats, mastermind: { attempted: false, numAttempts: game.previousGuesses.length, completed: game.completed } }
    ...
    private async revealWinner() {
        ...
        if (attempted) {
            const sortedUsers = usersWithStats.sort((a, b) => {
                const aBest = a.dailyGameStats?.mastermind?.numAttempts ?? 0
                const bBest = b.dailyGameStats?.mastermind?.numAttempts ?? 0
                return aBest - bBest
            })

            const bestScore = sortedUsers[0].dailyGameStats.mastermind.numAttempts
            const winners = sortedUsers
                .slice()
            ...
            const results = sortedUsers
                .map((user) => {
                    ...
                    const userResult = `${user.dailyGameStats.mastermind.completed ? user.dailyGameStats.mastermind.numAttempts : 'X'} / ${
                        GameValues.mastermind.totalAttempts
                    }`
                    return `${UserUtils.findUserById(user.id, this.client)}: ${userResult} ${isWinner ? ':crown:' : ''}`
                })
                .join('\n')
            ...
            if (winners && winners.length > 0) {
                ...
                for (const winner of winners) {
                    ...
                    winnerNames.push(UserUtils.findUserById(winner.id, this.client).username)
                }
                description =
