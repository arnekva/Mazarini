import { ActionRowBuilder, ButtonBuilder, ButtonInteraction, ButtonStyle, CacheType, ChatInputCommandInteraction, InteractionResponse, Message } from 'discord.js'
...
export class MoreOrLess extends AbstractCommands {
    ...
    public static async getNewMoreOrLessGame(): Promise<IMoreOrLess> {
        ...
        const games: IMoreOrLess[] = await (await fetch(url, {
            method: 'GET',
            headers: {
        ...
        const check: any = (await (await fetch(dataUrl, {
            method: 'GET',
            headers: {
                Accept: 'application/json',
            },
    ...
    private async fetchGameData() {
        ...
        const game: any = (await (await fetch(url, {
            method: 'GET',
            headers: {
                Accept: 'application/json',
            },
        })).json()).game
        const data: IMoreOrLessData[] = game.data.filter(item => item.length <= 4).map(item => {
    ...
    private async setupGame(interaction: ChatInputCommandInteraction<CacheType> | ButtonInteraction<CacheType>) {
        ...
        const embed = EmbedUtils.createSimpleEmbed(this.game.title, this.game.description).setThumbnail(`https://api.moreorless.io/img/${this.game.image}_512.jpg`)
    ...
    private updateGame(game: IMoreOrLessUserGame) {
        const description = `${game.current.subject} ${this.game.strings?.verb} **${TextUtils.formatLargeNumber(game.current.answer)}${this.game.strings?.valueSuffix ?? ''}** ${this.game.strings?.valueTitle}`
                          + `\n\nVS\n\n`
                          + `${game.next.subject}`
    ...
    private async endGame(game: IMoreOrLessUserGame, userId: string) {
        ...
        const description = `${game.next.subject} ${this.game.strings.verb} **${TextUtils.formatLargeNumber(game.next.answer)}${this.game.strings?.valueSuffix ?? ''}** ${this.game.strings.valueTitle}`
                          + `\n\n${msg}\n\n`
    ...
    private async revealResults(interaction: ChatInputCommandInteraction<CacheType>) {
        const embed = EmbedUtils.createSimpleEmbed('Dagens more or less resultater', this.game.title).setThumbnail(`https://api.moreorless.io/img/${this.game.image}_512.jpg`)
        ...
        for (const user of users) {
            ...
            const result = `Første forsøk: ${user.dailyGameStats.moreOrLess.firstAttempt} riktige`
...
const guessBtnRow = (btnMore: string = 'More', btnLess: string = 'Less') => new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder({
