...
export class VivinoCommands extends AbstractCommands {
    ...
    private async findData(interaction: ChatInputCommandInteraction<CacheType> | ButtonInteraction<CacheType>) {
        ...
        const cleanUrl = `https://api.vivino.com/v/9.0.0/users/56751223/activities?limit=100&start_from=`
        ...
        while (hasMore) {
            ...
            const localList = Object.values(localData) as any[]
        ...
        const listLength = flatList.length

        const newestItem = flatList[0] as IVivinoRating
        console.log(newestItem.object.image)
        ...
        const embed = EmbedUtils.createSimpleEmbed(
            ...
            .setFooter({ text: `Totalt ${listLength} ratinger` })

        this.messageHelper.replyToInteraction(interaction, embed, { hasBeenDefered: true })
    }

    private findRatingsThisYear(data: IVivinoRating[]) {
        const currYear = moment().year()

        const dataFromThisYear = data.filter((d) => moment(d.object.review.created_at).year() === currYear)
    }
