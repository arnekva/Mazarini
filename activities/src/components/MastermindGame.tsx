...
export function MastermindGame({ accessToken }: { accessToken: string }) {
      ...
      if (result.alreadyCompleted) {
        ...
        setStatus((s) =>
          s
            ? {
                ...s,
                guesses: [...s.guesses, { guess: current, black: result.black ?? 0, white: result.white ?? 0 }],
                numAttempts: result.numAttempts ?? s.numAttempts,
              }
            : s
        )
        if (result.solution) {
  ...
  return (
    <>
      <p className={contentStyles.status}>
