...
async function loadCustom(slug: string) {
  const file = CUSTOM_FILES[slug]
  if (!file) return null
  const local = [path.resolve(process.cwd(), "data/more-or-less/custom-games", file), path.resolve(process.cwd(), "../res/games/moreOrLess/customGames", file)].find((p) => fs.existsSync(p))
  let raw: any
...
function apiItems(data: unknown[]): MolItem[] { return data.filter((x): x is [string, number, string] => Array.isArray(x) && x.length <= 4).map((x) => ({ subject: x[0], answer: x[1], image: x[2] ?? "" })) }
...
function tierReward(from: number, to: number) { const r = moreOrLessValues.rewards; let total = 0; for (let i = from + 1; i <= to; i++) total += i <= 10 ? r.tier1 : i <= 20 ? r.tier2 : i <= 30 ? r.tier3 : i <= 40 ? r.tier4 : i <= 50 ? r.tier5 : r.tier6; return total }
...
export async function startMoreOrLessGame(user: AuthenticatedDiscordUser) {
  ...
  if (category.tags?.includes(CUSTOM_MOL_GAME_TAG)) { const custom = await loadCustom(category.slug); if (!custom) return Response.json({ error: "Klarte ikke å laste denne kategorien i appen" }, { status: 400 }); items = custom.items; strings = custom.strings }
  else { const response = await fetch(`https://api.moreorless.io/en/games/${category.slug}.json`, { headers: { Accept: "application/json" } }); if (!response.ok) return Response.json({ error: "Klarte ikke å hente kategori-data" }, { status: 502 }); const body = await response.json(); items = apiItems(body.game?.data ?? []); strings = body.game?.strings ?? strings }
  if (items.length < 2) return Response.json({ error: "For få elementer i kategorien" }, { status: 502 })
  const shuffled = shuffle(items); const current = shuffled.pop()!; const next = shuffled.pop()!; const session: MolSession = { slug: category.slug, data: shuffled, current, next, correctAnswers: 0 }
