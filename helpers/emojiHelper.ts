...
export type emojiType = "kekw_animated" | "catJAM";
export interface emojiReturnType {
    id: string;
    emojiObject?: GuildEmoji
}
export class EmojiHelper {
    static async getEmoji(emojiType: emojiType, message: Message): Promise<emojiReturnType> {
        let emojiObj;
        let animated = false;
        if (emojiType === "kekw_animated") {
            emojiObj = await message.client.emojis.cache.find(emoji => emoji.name == "kekw_animated");
            animated = true;
        }
