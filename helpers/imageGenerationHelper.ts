...
export class ImageGenerationHelper {
    ...
    private async getLightrayAndText(collectable: IUserCollectable): Promise<Buffer> {
        ...
        const resizedLightray = sharp(lightray).resize({fit: sharp.fit.cover, width: ratios.lightrayScaleWidth}).resize({fit: sharp.fit.cover, width: revealWidth, height: ratios.lightrayScaleHeight})
    ...
    private async getHeaderText(collectable: IUserCollectable, meta: sharp.Metadata): Promise<sharp.OverlayOptions[]> {
        ...
        const itemHeader = new UltimateTextToImage(header, {fontFamily: 'Work Sans', fontColor: '#ffffff', fontSize: fontSize, fontWeight: 500, margin: 2}).render().toBuffer(); 
        const resizedHeader = await sharp(itemHeader).resize({fit: sharp.fit.inside, height: ratios.textHeight}).toBuffer()
        
        const outline = new UltimateTextToImage(header, {fontFamily: 'Work Sans', fontColor: '#000000', fontSize: fontSize, fontWeight: 500, margin: 2}).render().toBuffer(); 
    ...
    private async overlayGif(background: Buffer, gifOverlay: Buffer) { //takes two input paths.
        ...
        const result = imgRoll.composite([
            { input: await overlay.toBuffer(), gravity: 'north', animated: true}
        ]).gif(
    ...
    private async getImageSeriesForCollectables(collectables: IUserCollectable[], imageTemplate: ICollectableImage): Promise<IImage[]> {
        ...
        const commonImages = await this.getImagesForRarity(collectables.filter(item => item.rarity === ItemRarity.Common), imageTemplate.common)
        images.push(...commonImages)
        const rareImages = await this.getImagesForRarity(collectables.filter(item => item.rarity === ItemRarity.Rare), imageTemplate.rare)
        images.push(...rareImages)
        const epicImages = await this.getImagesForRarity(collectables.filter(item => item.rarity === ItemRarity.Epic), imageTemplate.epic)
        images.push(...epicImages)
        const legendaryImages = await this.getImagesForRarity(collectables.filter(item => item.rarity === ItemRarity.Legendary), imageTemplate.legendary)
    ...
    private async getItemInventoryImage(item: IUserCollectable, color: ItemColor, coord: IItemShadowCoordinates): Promise<IImage> {
        ...
        const fontSize = 50
        const amountBuffer = new UltimateTextToImage(`${amount}`, {fontFamily: 'Work Sans', fontColor: '#ffffff', fontSize: fontSize, fontWeight: 500, margin: 2}).render().toBuffer(); 
        ...
        const amountImage = await sharp(resizedBg).composite([{ input: resizedAmount, top: Math.floor((bgMeta.height/2)-(amountMeta.height/2)), left: Math.floor((bgMeta.width/2)-(amountMeta.width/2)) }]).toBuffer()
